...
Template = ControlTable.DirectoryLocations[1]
TempDir = ControlTable.DirectoryLocations[2]
FinalDir = ControlTable.DirectoryLocations[3]
#####################################################################################################################
...
for line in ControlTable.values: # loop through each landscape_var in control table 
    ...
    #                    if not np.isnan(OldVal).any() and not np.isnan(NewVal).any() and DataType.count('Float'):
            ...
            # get extents from template to pass to gdalwarp
            NDV, Stats, xsize, ysize, GeoT, Proj_projcs, Proj_geogcs, DataType = getRasterInfo(Template)
            minx = GeoT[0]
            maxy = GeoT[3]
            maxx = minx + GeoT[1]*xsize
            miny = maxy + GeoT[5]*ysize
            bounds = str(minx) + " " + str(miny) + " " + str(maxx) + " " + str(maxy)
            template_projection = Proj_projcs
            dst_xsize = xsize
            dst_ysize = ysize
            dst_crs = 'EPSG:5070'
...
#                 Set Geoprocessing environments
                arcpy.env.snapRaster = '%s'%(Template)
                arcpy.env.cellSize = '%s'%(ConvertRes)
#                arcpy.env.mask = "L:\\Priv\\CORFiles\\Geospatial_Library\\Data\\RESOURCE\\POLITICAL\\BOUNDARIES\\NATIONAL\\Census_2000_State_State_Equivalents_Cartographic_Boundaries.shp"
                ...
                if not Proj_projcs==dst_crs and type(MaskRas) is str:
                    ...
                    final_ras = FinalDir + '/' + Rast + '.tif'
                    import arcpy
                    from arcpy.sa import *
                    arcpy.CheckOutExtension("Spatial")
                    arcpy.gp.ExtractByMask_sa(resamp_ras, MaskRas, final_ras)
                    outExtractByMask = ExtractByMask(resamp_ras, MaskRas)
#                     Save the output                    
                    outExtractByMask.save('L:/Priv/CORFiles/Geospatial_Library/Data/Project/SSWR1.1B/LandscapeRasters/QAComplete/roadden.tif')
            # examine results
            NDV, Stats, xsize, ysize, GeoT, Proj_projcs, Proj_geogcs, DataType = getRasterInfo(resamp_ras)
#            gdalwarp -overwrite -s_srs EPSG:5070 -q -cutline L:/Priv/CORFiles/Geospatial_Library/Data/Project/SSWR1.1B/LandscapeRasters/QAComplete/water_mask.tif -dstalpha -of GTiff C:/Users/mweber/Temp/roadden.tif C:/Users/mweber/Temp/test2.tif                
#        gdalwarp -q -cutline L:/Priv/CORFiles/Geospatial_Library/Data/RESOURCE/POLITICAL/BOUNDARIES/NATIONAL/Census_2000_State_State_Equivalents_Cartographic_Boundaries.shp -crop_to_cutline -of GTiff L:/Priv/CORFiles/Geospatial_Library/Data/Project/StreamCat/LandscapeRasters/QAComplete/roadden.tif C:/Users/mweber/Temp/roadden_3.tif
