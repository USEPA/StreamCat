...
for i in os.listdir(lookupdir):
    if i.count('.csv') and not i.count('State_VPU') and not i.count('states_lookup') and not i.count('COMID_State'):
        ...
        for VPU in state_lookup[ST_NM]:
            if count == 0:
                outtable = pd.read_csv('L:/Priv/CORFiles/Geospatial_Library_Projects/StreamCat/FTP_Staging/StreamCat/HydroRegions/' + var + '_Region' + VPU + '.csv')
                outtable = outtable[outtable['COMID'].isin(state['COMID'])]
            if count > 0:
                temp = pd.read_csv('L:/Priv/CORFiles/Geospatial_Library_Projects/StreamCat/FTP_Staging/StreamCat/HydroRegions/' + var + '_Region' + VPU + '.csv')
        ...
        outtable.to_csv('L:/Priv/CORFiles/Geospatial_Library_Projects/StreamCat/FTP_Staging/StreamCat/States/' + var + '_' + St_Abbr_lookup[ST_NM] + '.csv',index=False)
