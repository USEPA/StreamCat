...
@click.command()
@click.option('--use_arcpy', type=click.BOOL, default=True, help='Whether or not using an arcpy environments')
@click.option('--use_dask', type=click.BOOL, default=True, help='Whether or not to use dask for large computations. If this is true use_arcpy should be false.')
...
def StreamCat_CLI(use_arcpy, use_dask, num_workers, control_table, write_to_db):
    ...
    if not os.path.exists(args.ACCUM_DIR):
        ...
        nhd = NHDProcessing(args.NHD_DIR, args.ACCUM_DIR)
    ...
    with click.progressbar(iterable=ctl.query("run == 1").iterrows(), label="Processing Metrics") as bar:
        for idx, row in bar:
            ...
            # TODO define a dask cluster as well and pass it to process metrics as well.
            process_metric(args, row, inter_vpu, INPUTS, use_arcpy, use_dask, num_workers)
